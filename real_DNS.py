...
def check_domain_to_ip(domain):
    ...
    if domain in domain_dict:
        ...
        try:
            ...
            domain_ip = socket.gethostbyname(domain) #DNS has to have import socket eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
...
def handle_dns_request(pkt):
    ...
    if pkt.haslayer(DNSQR) and pkt.getlayer(UDP).dport == 53 and pkt[IP].dst == IP_DNS:
        ...
        print(f"Received DNS query for {qname} ({dnsqtypes.get(qtype)})")
        ip = check_domain_to_ip(qname.decode('utf-8')[:-1])
        print(ip)
